...
export default function TripPage() {
  ...
  const handleAddExpense = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewExpense({ category: '', description: '', amount: 0, currency: 'USD' });
  ...
  const handleShare = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api.post(`/trips/${tripId}/share`, { email: shareEmail, permission: sharePermission });
      setShareEmail('');
      ...
      alert('Failed to share trip. Please check the email and try again.');
    }
  };
      ...
      {showShareModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4">
            <h3 className="text-xl font-bold mb-4">Share Trip</h3>
