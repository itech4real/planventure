...
class JWTUtility:
    ...
    def create_tokens(user_id, additional_claims=None):
        ...
        try:
            ...
            # Fallback to PyJWT when outside Flask app context
            secret = os.getenv('JWT_SECRET_KEY', 'your-secret-key-change-in-production')
            now = datetime.utcnow()
            access_payload = {'sub': str(user_id), **claims, 'iat': now, 'exp': now + timedelta(hours=JWTUtility.ACCESS_TOKEN_EXPIRES)}
            refresh_payload = {'sub': str(user_id), **claims, 'iat': now, 'exp': now + timedelta(days=JWTUtility.REFRESH_TOKEN_EXPIRES)}
    ...
    def create_access_token(user_id, additional_claims=None):
        ...
        try:
            ...
            secret = os.getenv('JWT_SECRET_KEY', 'your-secret-key-change-in-production')
            now = datetime.utcnow()
            payload = {'sub': str(user_id), **claims, 'iat': now, 'exp': now + timedelta(hours=JWTUtility.ACCESS_TOKEN_EXPIRES)}
    ...
    def create_refresh_token(user_id, additional_claims=None):
        ...
        try:
            ...
            secret = os.getenv('JWT_SECRET_KEY', 'your-secret-key-change-in-production')
            now = datetime.utcnow()
            payload = {'sub': str(user_id), **claims, 'iat': now, 'exp': now + timedelta(days=JWTUtility.REFRESH_TOKEN_EXPIRES)}
            return pyjwt.encode(payload, secret, algorithm='HS256')
