...
db.init_app(app)
# Initialize JWT, CORS and rate limiting
...
@app.route('/')
def index():
    return jsonify({"message": "PlanVenture API is running!"})
...
print("About to define registration route")

@app.route('/auth/register', methods=['POST'])
def register():
    print("Registration endpoint called - POST")
    ...
    is_strong, feedback = PasswordUtility.is_password_strong(data['password'])
    if not is_strong:
        return jsonify({"error": "Weak password", "details": feedback}), 400
...
print("Registration route defined successfully")
...
def login():
    ...
    try:
        tokens = JWTUtility.create_tokens(user.id)
        return jsonify({**user.to_dict(), **tokens}), 200
    except Exception as e:
